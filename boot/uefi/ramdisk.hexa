...
efi::EFI_STATUS loadRamDiskFromVolume(efi::EFI_BOOT_SERVICES *bootsvc, efi::EFI_FILE_PROTOCOL *root,
	...
	serialPrintf(L"[[[efi_main.loadRamDiskFromVolume]]] status: Open %d\n", status);
	...
	serialPrintf(L"[[[efi_main.loadRamDiskFromVolume]]] status: GetInfo %d\n", status);

	uint64_t size = ((efi::EFI_FILE_INFO *)info)->FileSize;
	serialPrintf(L"[[[efi_main.loadRamDiskFromVolume]]] FileSize %d\n", size);
	...
	serialPrintf(L"[[[efi_main.loadRamDiskFromVolume]]] status: Close %d\n", status);
	...
	serialPrintf(L"[[[efi_main.loadRamDiskFromVolume]]] checksum: %d expected: %d\n", checksum, uints[0]);
	if (checksum != uints[0])
		serialPrintln(
			L"[[[efi_main.loadRamDiskFromVolume]]] <ERROR> checksum is incorrect, disk may be corrupted!");
...
efi::EFI_STATUS findAndLoadRamDisk(efi::EFI_BOOT_SERVICES *bootsvc, RamDisk *ramdisk) {
	...
	status = bootsvc->LocateHandleBuffer(efi::ByProtocol, &simpleFileSystemProtocol, NULL, &handleCount,
										 &handleBuffer);
	if (status != EFI_SUCCESS)
		serialPrintf(L"[[[efi_main.findAndLoadRamDisk]]] <ERROR> failed: LocateHandleBuffer %d\n", status);
	else
		serialPrintf(L"[[[efi_main.findAndLoadRamDisk]]] success: LocateHandleBuffer, got %d handles\n",
					 handleCount);

	for (uint64_t i = 0; i < handleCount; ++i) {
		serialPrintf(L"[[[efi_main.findAndLoadRamDisk]]] loading handle #%d of %d handles\n", i, handleCount);
		...
		if (status != EFI_SUCCESS) {
			serialPrintf(
				L"[[[efi_main.findAndLoadRamDisk]]] HandleProtocol got a non-file system (status %d, handle "
				L"%d), continue\n",
				status, i);
			continue;
		}

		serialPrintln(L"[[[efi_main.findAndLoadRamDisk]]] success: HandleProtocol found a file system");
		efi::EFI_FILE_PROTOCOL *root = NULL;
		status = fileSystem->OpenVolume(fileSystem, &root);
		if (status != EFI_SUCCESS) {
			serialPrintf(L"[[[efi_main.findAndLoadRamDisk]]] failed: OpenVolume with status %d, continue to "
						 L"the next one\n",
						 status);
			continue;
		}
		...
		if (status != EFI_SUCCESS) {
			serialPrintf(
				L"[[[efi_main.findAndLoadRamDisk]]] failed: loadRamDiskFromVolume with status %d, continue "
				L"to the next one\n",
				status);
			continue;
		}
	...
	serialPrintf(L"[[[efi_main.findAndLoadRamDisk]]] <ERROR> failed to find RAM disk at all\n");
